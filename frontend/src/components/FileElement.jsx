...
export default function FileElement({ filename, utils }) {
  ...
  async function handleDelete() {
    try {
      ...
      const resp = await axios.delete("http://localhost:8008/delete", {
        params: {
          filename
        }
      })
  ...
  async function handleDownload() {
    const resp = await axios.get("http://localhost:8008/retrieve", {
      params: {
        filename: filename
      },
    })
  ...
  return (
    ...
    <div className="">
      ...
      <button style={{color: "blue"}} onClick={() => {window.open(`http://localhost:8008/retrieve?filename=${filename}`)}}>View</button>
